...
static const u8 sMenuText_Tap[] = _("TAP + DPAD");
    ...
    default:
        ...
        else
        {
            ...
            case KEYITEMS_POCKET:
                ...
                if (RegisteredItemIndex(gSpecialVar_ItemId) >= 0 || gSaveBlock1Ptr->registeredLongItem == gSpecialVar_ItemId)
...
static void ItemMenu_Register(u8 taskId)
{
    ...
    // Handle regular registered items array
...
bool8 UseRegisteredKeyItemOnField(bool8 isRegisterHold)
{
    ...
    // Handle long press registration
    if (isRegisterHold) {
        item = gSaveBlock1Ptr->registeredLongItem;
        if (item != ITEM_NONE) {
            if (CheckBagHasItem(item, 1) == TRUE) {
                gSpecialVar_ItemId = item;
                func = ItemId_GetFieldFunc(item);
            } else {
                gSaveBlock1Ptr->registeredLongItem = ITEM_NONE;
            }
        }
    ...
    // Handle regular registered items with wheel
